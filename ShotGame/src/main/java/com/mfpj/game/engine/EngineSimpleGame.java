...
public class EngineSimpleGame extends BaseGameActivity implements
    ...
    private ChangeableText phaseText;
    ...
    public EngineSimpleGame toastEngineSimpleGame = EngineSimpleGame.this;
	...
	public Engine onLoadEngine() {
        ...
        int cameraWidth  = size.x;
        int cameraHeight = size.y;
	...
	public void onLoadResources() {
		...
		BitmapTextureAtlas mBitmapTextureAtlas = new BitmapTextureAtlas(512, 512,
		...
		mPlayerTextureRegion = BitmapTextureAtlasTextureRegionFactory
				.createFromAsset(mBitmapTextureAtlas, this, "Player.png",
						0, 0);
		...
		mTargetTextureRegion = BitmapTextureAtlasTextureRegionFactory
				.createFromAsset(mBitmapTextureAtlas, this, "Target.png",
						128, 0);
		...
		try {
			shootingSound = SoundFactory.createSoundFromAsset(mEngine
					.getSoundManager(), this, "shot_pew_pew.wav");
	...
	public Scene onLoadScene() {
		...
		// set background color 255,255,255 = White
		mMainScene = new Scene();
		mMainScene
				.setBackground(new ColorBackground(255f, 255f, 255f));
		...
		score = new ChangeableText(0, 0, mFont, "Kills : " + String.valueOf(maxScore));
		// repositioning the score later so we can use the score.getWidth()
		score.setPosition(mCamera.getWidth() - score.getWidth() - 5, 5);

        //showing current phase of the game
        phaseText = new ChangeableText(0, 0, mFont, "Phase " + String.valueOf(phaseNumber));
        phaseText.setPosition(5, 5);
		...
		public void onUpdate(float pSecondsElapsed) {
			...
			while (targets.hasNext()) {
				...
				if (hit) {
					...
					hit = false;
					hitCount++;
					score.setText("Kills : " + String.valueOf(hitCount));
				}
			}
			...
			if (hitCount >= maxScore) {
				phaseNumber++;
                phaseText.setText("Phase " + String.valueOf(phaseNumber));
                maxScore+=10;
	...
	private void shootProjectile(final float pX, final float pY) {
		...
		float length = (float) Math.sqrt((offRealX * offRealX)
				+ (offRealY * offRealY));
		float velocity = 900.0f / 1.0f; // 480 pixels / 1 sec
	...
	public void restart() {
        ...
        toastEngineSimpleGame.runOnUiThread(new Runnable() {
            public void run() {
                Toast.makeText(toastEngineSimpleGame.getBaseContext(), "Phase "+ phaseNumber + " | Kill " + maxScore + " enemies", Toast.LENGTH_SHORT).show();
            }
        });
		...
		runOnUpdateThread(new Runnable() {
			...
			public void run() {
                ...
                mMainScene.attachChild(phaseText);
			}
		});
	...
	public void onResumeGame() {
		...
		if (runningFlag) {
			if (pauseFlag) {
				Toast.makeText(this, "Click to resume",
    ...
    public void onBackPressed() {
        ...
        if (!pauseFlag && mEngine.isRunning() && backgroundMusic.isPlaying()) {
            Toast.makeText(this, "Click to resume or click again to exit",
                    Toast.LENGTH_SHORT).show();
            pauseMusic();
            pauseFlag = true;
            pauseGame();
        }else if (failSprite.isVisible() && !mEngine.isRunning() && backgroundMusic.isPlaying()) {
            mMainScene.clearChildScene();
            mEngine.start();
            restart();
        }else if (winSprite.isVisible() && !mEngine.isRunning() && backgroundMusic.isPlaying()) {
            mMainScene.clearChildScene();
            mEngine.start();
            restart();
        }else{
